...
foreach (['cfg/var', 'app', 'tools', 'public/views/themes/default', 'public/views/themes/store-theme', 'public/static/img/2026', 'plugins/local-plugin', 'private_files/media/2026'] as $directory) {
...
try {
    ...
    $check(str_contains($page, 'site-health__clean-ring')
        && str_contains($page, 'max((int)$summary[\'expected\'], $observedTotal)'), 'dashboard presents a conservative clean Core percentage ring');
    foreach (['Run full scan', 'Scanned', 'Plugin and Theme Inventory', 'Media and File Safety', 'Inventory and filesystem safety', 'Executable and MIME safety rules'] as $source) {
