...
- `cfg/helpers/site_health.php` orchestrates the shared-lock scan. Canonical Store plugins and non-system themes are hashed against the exact installed-version release manifest; local manifest URLs, local extensions, and noncanonical Store origins remain `unverified`. Extension identity is file-only, so ordinary empty directories are ignored. A bounded regular JSON-object `.store.json` is updater-preserved metadata outside package identity and grants no trust; `.git` is always prohibited. Plugins may declare at most eight `site_health.generated_public_image_directories`, all confined to `static/plugins/{folder}/`; only bounded regular non-executable images with an allowlisted extension and matching MIME bypass immutable release hashes. Invalid declarations and unsafe generated files fail closed, while packaged code and copied static assets remain exact-hash verified. Extension release baselines are exact HTTPS Store manifests, not signatures. Hook usage does not modify Core. Media/files report `scanned` when bounded executable, symlink, special-file, and image-MIME checks complete without a finding; this is not a malware-free guarantee.
