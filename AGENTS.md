...
- `cfg/helpers/site_health.php` orchestrates the shared-lock scan. Canonical Store plugins and non-system themes are hashed against the exact installed-version release manifest; local manifest URLs, local extensions, and noncanonical Store origins remain `unverified`. Extension identity is file-only, so ordinary empty directories are ignored. A bounded regular JSON-object `.store.json` is updater-preserved metadata outside package identity and grants no trust; `.git` is always prohibited. Extension release baselines are exact HTTPS Store manifests, not signatures. Hook usage does not modify Core. Media/files report `scanned` when bounded executable, symlink, special-file, and image-MIME checks complete without a finding; this is not a malware-free guarantee.
