...
### Theme operation and update extension contract
...
- `theme_manager_theme_actions($themeRow, $completePhysicalManifest, $context)` renders extension-owned actions in each installed theme card. Context includes folder/state flags, cached update metadata, user ID, admin base path, and return URL.
...
- Lifecycle observers use `do_action_isolated()` so every listener runs even if an earlier listener throws. The helper returns structured errors for caller logging.
- `plugin_state_change_preflight($state, $name, $operation)` is enforced centrally by plugin disable and delete/uninstall operations. State is exactly `['allowed' => bool, 'message' => string]`; malformed output or exceptions deny the operation and populate `plugin_last_error()`.
- Theme and plugin updates build validated private trees beside the live directory and publish portably while holding the global exclusive lifecycle lock. Publication renames the exact old tree to a unique same-parent `.package-publication-recovery-*` path, renames the complete stage to the target, verifies both identities, and retains the old path until all post-publication work succeeds. Rollback uses the same guarded two-rename sequence and exact identity checks. Shared request readers and other writers remain blocked, so cooperating requests observe only a complete old or complete new executable tree.
- A process killed between the two renames can leave the target temporarily absent and the known-good old tree in the clearly named recovery path. The next lifecycle publication detects any such path and fails closed without deleting it; an operator must inspect and restore or archive the exact tree manually. This narrow process-kill residual is the only missing-target window for cooperating deployments. Fresh plugin installs still persist disabled state before their single-rename publication.
...
- Applied filenames and migration content never change or disappear. Core preserves the original SHA-256 ledger and accepts only exact bytes or a whole-file LF/CRLF checkout conversion; mixed endings, lone carriage returns, and all other byte changes fail closed. Migrations are forward-only, idempotent, and compatible with the prior plugin version because MySQL/MariaDB DDL may survive failure and file rollback.
...
- Deployments may declare expected non-executable public assets and local `tools/` files in preserved `cfg/site-files.json` using schema `1` and an exact logical-path to SHA-256 map. Matching files remain outside Core ownership; hash changes, unsafe file types, malformed manifests, Core runtime paths, and public executable files fail closed.
- Deployments may optionally configure the all-or-none set `SITE_HEALTH_DEPLOYMENT_MANIFEST_PATH` (an absolute private regular-file path), `SITE_HEALTH_DEPLOYMENT_PUBLIC_KEY` (strict base64 for a raw 32-byte Ed25519 public key), and `SITE_HEALTH_DEPLOYMENT_SOURCE_REVISION` (the exact deployed 40-lowercase-hex revision). The schema-1 envelope contains canonical JSON payload bytes and a detached signature over `Jyavani deployment extension manifest\0v1\0` plus those exact bytes. Payloads bind an optional opaque deployment identifier, that exact runtime source revision, and sorted unique plugin/theme entries with installed version and exact sorted file maps. Keep the private key off the CMS host. Missing configuration preserves normal behavior only when all three values are empty; partial/invalid configuration, revision mismatch, invalid signatures, malformed payloads, or missing sodium fail closed to unverified for otherwise local extensions.
...
- `cfg/helpers/site_health.php` orchestrates the shared-lock scan. Canonical Store plugins and non-system themes are hashed against the exact installed-version release manifest; that baseline takes precedence. Extensions without a canonical Store baseline may use a matching signed local deployment entry. Extension identity is file-only, so ordinary empty directories are ignored. A bounded regular JSON-object `.store.json` is updater-preserved metadata outside package identity and grants no trust; `.git` is always prohibited. Plugins may declare at most eight `site_health.generated_public_image_directories`, all confined to `static/plugins/{folder}/`; only bounded regular non-executable images with an allowlisted extension and matching MIME bypass immutable release hashes. Invalid declarations and unsafe generated files fail closed, while packaged code and copied static assets remain exact-hash verified. Hook usage does not modify Core. Media/files report `scanned` when bounded executable, symlink, special-file, and image-MIME checks complete without a finding; this is not a malware-free guarantee.
- Authorized Site Owner dashboard loads asynchronously request a scan when the validated report is missing, belongs to another Core version, or is at least one hour old. Browser throttling limits repeated attempts, and `site_health_run_and_store_if_stale()` rechecks freshness only after obtaining the nonblocking scan lock. The dashboard render path remains read-only, and the automatic scan endpoint releases the PHP session before hashing. The synchronous manual page retains its session because it renders a fresh CSRF token after the scan.
- The Site Health home widget is required whenever the current Site Owner has `core.settings.manage`. Default and legacy layouts append it on the right, the renderer omits its Hide control, and layout saves preserve it. Its count describes integrity findings, not confirmed vulnerabilities.
