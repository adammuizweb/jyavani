...
class PostController
{
    ...
    public static function resolve_post_display_image(array $post): ?string
    {
        if (!empty($post['youtube'])) {
            $yt = self::youtube_thumb_url($post['youtube']);
            if ($yt) return $yt;
        }
        if (!empty($post['display_image'])) return $post['display_image'];
        if (!empty($post['thumbnail'])) return $post['thumbnail'];
        return null;
    }
    ...
    exit;
        ...
        switch ($type) {
            ...
            case 'page':
                ...
                PageController::renderPage($pdo, $row['slug'] ?? '');
                break;
...
public static function listArticles(PDO $pdo, int $page = 1, string $q = '')
{
    ...
    if (trim($content_html) === '') {
        try {
            ...
            $resolved = ['type' => 'theme_file', 'theme_folder' => DEFAULT_THEME_FOLDER, 'theme_file' => $themeFile];
    ...
    if (trim($content_html) === '') {
        ...
        <div class="container">
          ...
          <?php if (!empty($vars['q'])): ?>
            <p>Hasil pencarian untuk: <strong><?= htmlspecialchars($vars['q'], ENT_QUOTES, 'UTF-8') ?></strong></p>
          <?php endif; ?>
            ...
            <?php foreach ($posts as $p): ?>
              <article class="post-item" style="margin-bottom:1.5rem;padding-bottom:1rem;border-bottom:1px solid #eee">
                ...
                <?php if (!empty($p['display_image'])): ?>
                  <img src="<?= htmlspecialchars($p['display_image'], ENT_QUOTES, 'UTF-8') ?>"
                       alt=""
...
protected static function renderArticle(array $row, PDO $pdo)
{
    ...
    if (trim($content_html) === '') {
        ...
        ?>
        <article>
          <h1><?= htmlspecialchars($postData['title'] ?? '', ENT_QUOTES, 'UTF-8') ?></h1>
          <div><?= $postData['content'] ?? '' /* content already assumed safe/encoded by caller if needed */ ?></div>
        </article>
        <?php
        $content_html = ob_get_clean();
    }
    ...
    $canonical_url = $scheme . '://' . $host . '/' . rawurlencode($postData['slug'] ?? '') . '/';
