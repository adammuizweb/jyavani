...
<?php if ($tab === 'presets'): ?>
  <p style="margin-bottom:1rem">
    <a class="adam-button" href="<?= h($base . '/index.php?page=admin/shortcodes/edit') ?>">+ Tambah Preset</a>
    <?php if ($isAdmin): ?>
      &nbsp;&nbsp;
      <a class="adam-att" href="<?= h($base . '/index.php?page=admin/bin/index') ?>">🗑️ Trash</a>
    <?php endif; ?>
  </p>
    ...
    <table class="adam-table">
      ...
      <tbody>
        <?php if (empty($presets)): ?>
          <tr><td colspan="5" style="padding:1rem;">Belum ada preset. <a href="<?= h($base . '/index.php?page=admin/shortcodes/edit') ?>">Buat sekarang</a>.</td></tr>
        <?php else: ?>
          <?php foreach ($presets as $p): ?>
            <?php
              $st = strtolower(trim((string)($p['status'] ?? 'draft')));
              $stClass = in_array($st, ['published','draft','private'], true) ? $st : 'unknown';
              $editHref = $base . '/index.php?' . http_build_query(['page' => 'admin/shortcodes/edit', 'id' => (int)$p['id']]);
            ?>
            <tr class="adam-row">
  ...
  <form id="preset-delete-form" method="post" action="<?= h($base . '/admin/shortcodes/delete.php') ?>" style="display:none;">
    ...
    <input type="hidden" name="return_to" value="<?= h($base . '/index.php?page=admin/shortcodes/index&tab=presets') ?>">
  </form>

<?php elseif ($tab === 'layouts'): ?>
  <p style="margin-bottom:1rem">
    <a class="adam-button" href="<?= h($base . '/index.php?page=admin/shortcodes/layout') ?>">+ Tambah Layout</a>
  </p>
    ...
    <table class="adam-table">
      ...
      <tbody>
        <?php if (empty($layoutFiles)): ?>
          <tr><td colspan="4" style="padding:1rem;">Belum ada layout template. <a href="<?= h($base . '/index.php?page=admin/shortcodes/layout') ?>">Buat sekarang</a>.</td></tr>
        <?php else: ?>
          <?php foreach ($layoutFiles as $f):
            ...
            $editHref = $base . '/index.php?' . http_build_query(['page' => 'admin/shortcodes/layout', 'file' => $f]);
          ?>
            <tr class="adam-row">
  ...
  <form id="layout-delete-form" method="post" action="<?= h($base . '/admin/shortcodes/delete_layout.php') ?>" style="display:none;">
    ...
    <input type="hidden" name="return_to" value="<?= h($base . '/index.php?page=admin/shortcodes/index&tab=layouts') ?>">
  </form>
<?php endif; ?>
</section>
