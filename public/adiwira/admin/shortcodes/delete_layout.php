...
if (!adiwira_csrf_validate($csrf)) {
    adiwira_redirect_with_flash((string)($_POST['return_to'] ?? '/adiwira/index.php?page=admin/shortcodes/index&tab=layouts'), 'error', 'CSRF invalid.');
}

$base = rtrim(str_replace('\\','/', dirname($_SERVER['SCRIPT_NAME'])), '/');
$return_to = function_exists('adiwira_safe_return_to')
    ? adiwira_safe_return_to((string)($_POST['return_to'] ?? ''), $base . '/index.php?page=admin/shortcodes/index&tab=layouts')
    : ($base . '/index.php?page=admin/shortcodes/index&tab=layouts');
