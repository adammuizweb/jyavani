...
if ($user_role !== 'admin') {
    http_response_code(403);
    exit('<p>Akses ditolak: menu Themes hanya untuk admin.</p>');
}
 ...
 * - Added 'sidebar' (complementary), 'main.search' and 'main.404' to match theme_helper mappings.
...
<div class="tm-wrap">
  ...
  <?php if ($messages): ?>
    <div class="tm-alert tm-alert--ok">
      <ul style="margin:0;padding-left:18px">
        <?php foreach($messages as $m) echo '<li>'.htmlspecialchars($m,ENT_QUOTES,'UTF-8').'</li>'; ?>
      </ul>
    </div>
  <?php endif; ?>

  <!-- Controls: Scan + Upload -->
          ...
          <div class="tm-delrow">
            ...
            <?php elseif ($user_role === 'admin'): ?>
              <form method="post" style="display:flex;gap:8px;align-items:center;margin:0"
                    onsubmit="return confirm('Hapus tema <?= htmlspecialchars($folder, ENT_QUOTES, 'UTF-8') ?> dari database? Assignment yang menunjuk akan dibersihkan.');">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(function_exists('csrf_token') ? csrf_token() : '', ENT_QUOTES, 'UTF-8') ?>">
            ...
            <?php else: ?>
              <span class="tm-protected" title="Hanya admin yang bisa menghapus">Protected</span>
            <?php endif; ?>
          </div>
          ...
          <div class="tm-actions">
            <?php if (!$isActive): ?>
              <form method="post" style="display:inline;margin:0"
                    onsubmit="return confirm('Activate and apply <?= htmlspecialchars($folder, ENT_QUOTES, 'UTF-8') ?> to all slots?');">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(function_exists('csrf_token') ? csrf_token() : '', ENT_QUOTES, 'UTF-8') ?>">
            ...
            <form method="post" style="display:inline;margin:0"
                  onsubmit="return confirm('Apply theme <?= htmlspecialchars($folder, ENT_QUOTES, 'UTF-8') ?> to ALL slots?');">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(function_exists('csrf_token') ? csrf_token() : '', ENT_QUOTES, 'UTF-8') ?>">
    ...
    <form method="post" id="theme-assign-form" style="margin-top:12px">
      ...
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;margin-top:12px">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <a class="tm-ghost" href="?" style="text-decoration:none;display:inline-flex;align-items:center">Cancel</a>
        </div>
