...
// lokasi file /adiwira/admin/media/delete.php
ob_start();
...
if ($id > 0) {
    ...
    if ($r && !empty($r['url'])) $url = (string)$r['url'];
}

$deleted_id = $id;

// If id not provided, try find id by url
if ($deleted_id <= 0 && $url !== '') {
...
// Delete physical file (only under /static/)
...
try {
    ...
    adiwira_json(['ok'=>true,'id'=>$deleted_id], 200);
