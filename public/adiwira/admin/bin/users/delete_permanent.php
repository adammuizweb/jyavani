...
if ($role !== 'admin') {
  qz_redirect($back . '&err=' . urlencode('Akses ditolak: hanya admin yang boleh hapus permanen user.'));
}
...
try {
  $pdo->prepare("DELETE FROM users WHERE id = :id AND is_deleted = 1 LIMIT 1")
      ->execute([':id' => $id]);

  qz_redirect($back . '&msg=' . urlencode('User berhasil dihapus permanen 🧨'));
} catch (Throwable $e) {
