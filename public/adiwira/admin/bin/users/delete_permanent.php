...
if ($role !== 'admin') {
    adiwira_redirect_with_flash($returnTo, 'error', 'Akses ditolak: hanya admin yang boleh hapus permanen user.');
}
...
try {
    ...
    adiwira_redirect_with_flash($returnTo, 'success', 'User berhasil dihapus permanen.');

} catch (Throwable $e) {
    error_log('bin/users/delete_permanent.php error: ' . $e->getMessage());
    adiwira_redirect_with_flash($returnTo, 'error', 'Gagal hapus permanen user.');
}