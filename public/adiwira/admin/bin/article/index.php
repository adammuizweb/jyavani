...
// /adiwira/admin/bin/article/index.php
...
<section class="adam-card">
  ...
  <form method="get" style="margin-bottom:1rem;display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
    ...
    <a href="<?= htmlspecialchars($base . '/index.php?page=admin/bin/article/index', ENT_QUOTES, 'UTF-8') ?>" class="adam-cancle">Reset</a>
    ...
    <form id="binArticleBulkForm" method="post" action="<?= htmlspecialchars($base . '/admin/bin/article/bulk_action.php', ENT_QUOTES, 'UTF-8') ?>">
      ...
      <div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.6rem;flex-wrap:wrap;">
        <label style="display:flex;align-items:center;gap:.4rem;">
          <input type="checkbox" id="selectAllBinArticle"> Pilih semua di halaman
        </label>
          ...
          <?php else: ?>
            <?php foreach ($posts as $p): ?>
              ...
              <tr class="adam-row">
                ...
                <td><?= htmlspecialchars(format_date_ddmmyyyy_time_bracket((string)($p['created_at'] ?? '')), ENT_QUOTES, 'UTF-8') ?></td>
  ...
  <?php if ($pages > 1): ?>
    <nav class="adam-pagination" style="margin-top:1rem;">
      <?php foreach ($paging_items as $item):
        ...
        $query = $_GET;
        $query['p'] = $i;
        $query['page'] = 'admin/bin/article/index';
  ...
  <form id="bin-article-restore-form" method="post" action="<?= htmlspecialchars($base . '/admin/bin/article/restore.php', ENT_QUOTES, 'UTF-8') ?>" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" name="id" id="bin-article-restore-id">
    <input type="hidden" name="return_to" id="bin-article-restore-return-to" value="<?= htmlspecialchars($currentReturnTo, ENT_QUOTES, 'UTF-8') ?>">
  </form>

  <form id="bin-article-delete-form" method="post" action="<?= htmlspecialchars($base . '/admin/bin/article/delete_permanent.php', ENT_QUOTES, 'UTF-8') ?>" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" name="id" id="bin-article-delete-id">
    <input type="hidden" name="return_to" id="bin-article-delete-return-to" value="<?= htmlspecialchars($currentReturnTo, ENT_QUOTES, 'UTF-8') ?>">
  </form>
</section>
...
(function(){
  ...
  const restoreReturnTo = document.getElementById('bin-article-restore-return-to');

  const deleteForm = document.getElementById('bin-article-delete-form');
  const deleteId = document.getElementById('bin-article-delete-id');
  const deleteReturnTo = document.getElementById('bin-article-delete-return-to');
  ...
  function getBulkSummary(){
    ...
    if (count < 1) {
      return { ok:false, message:'Pilih minimal satu artikel.' };
    }
    ...
    if (action === 'restore') {
      return {
        ok: true,
        variant: 'warning',
        title: 'Restore artikel terpilih',
        message: 'Sebanyak ' + count + ' artikel akan direstore dari trash. Lanjutkan?',
    ...
    if (action === 'delete_permanent') {
      return {
        ...
        message: 'Sebanyak ' + count + ' artikel akan dihapus permanen. Aksi ini tidak bisa dibatalkan.',
  ...
  document.querySelectorAll('.js-bin-article-restore').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      ask('warning', {
        ...
        message: 'Restore artikel "' + title + '" dari trash?',
        confirmText: 'Ya, restore',
        cancelText: 'Batal'
      }).then(function(ok){
        if (!ok) return;
  ...
  document.querySelectorAll('.js-bin-article-delete-permanent').forEach(function(btn){
    btn.addEventListener('click', function(){
      const id = this.getAttribute('data-id') || '';
      const title = this.getAttribute('data-title') || 'artikel ini';
      const returnTo = this.getAttribute('data-return-to') || '';
      ...
      ask('danger', {
        ...
        message: 'Hapus permanen artikel "' + title + '"? Aksi ini tidak bisa dibatalkan.',
        confirmText: 'Ya, hapus permanen',
        cancelText: 'Batal'
      }).then(function(ok){
        if (!ok) return;
