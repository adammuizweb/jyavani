<?php
// lokasi file /adiwira/admin/bin/article/index.php
...
SELECT
  ...
  u.name AS created_by, u.username AS author_username,
...
$canBulk = ($role !== 'author'); // bulk disembunyikan untuk author
?>
...
<section class="adam-card">
  ...
  <form method="get" style="margin-bottom:1rem;display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
    ...
    <a href="<?= htmlspecialchars($root . '/index.php?page=admin/bin/article/index', ENT_QUOTES, 'UTF-8') ?>" class="adam-link">Reset</a>
  ...
  <?php if ($canBulk): ?>
    <form method="post" action="<?= htmlspecialchars($root . '/admin/bin/article/bulk_action.php', ENT_QUOTES, 'UTF-8') ?>" onsubmit="return confirmBinBulk()">
      ...
      <?php else: ?>
        <?php foreach ($posts as $p): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($p['categories'] ?? '-', ENT_QUOTES, 'UTF-8') ?></td>

            <td>
              <?= htmlspecialchars(!empty($p['deleted_at']) ? format_date_ddmmyyyy_time_bracket($p['deleted_at']) : '-', ENT_QUOTES, 'UTF-8') ?>
            </td>
...
function openBinModal(type, btn) {
  ...
  if (type === 'restore') {
    document.getElementById('binTitle').innerText = 'Restore Artikel';
    document.getElementById('binText').innerText = `Restore artikel "${title}"?`;
    document.getElementById('binSubmit').innerText = 'Restore';
    document.getElementById('binSubmit').style.background = '#246';
    form.action = "<?= htmlspecialchars($root . '/admin/bin/article/restore.php', ENT_QUOTES, 'UTF-8') ?>";
  } else {
    document.getElementById('binTitle').innerText = 'Hapus Permanen';
    document.getElementById('binText').innerText = `Hapus permanen artikel "${title}"? Ini tidak bisa dibatalkan.`;
    document.getElementById('binSubmit').innerText = 'Hapus Permanen';
    document.getElementById('binSubmit').style.background = '#c33';
    form.action = "<?= htmlspecialchars($root . '/admin/bin/article/delete_permanent.php', ENT_QUOTES, 'UTF-8') ?>";
  }
...
function confirmBinBulk() {
  ...
  if (!any) { alert('Pilih minimal satu artikel.'); return false; }

  if (action === 'delete_permanent') {
    return confirm('Yakin ingin menghapus permanen semua artikel terpilih? Ini tidak bisa dibatalkan.');
  }
  return true;
}
</script>
