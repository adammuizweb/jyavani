...
if ($role === 'author' && (int)($post['created_by'] ?? 0) !== $uid) {
    adiwira_redirect_with_flash($returnTo, 'error', 'Role kamu tidak punya akses restore artikel ini.');
}
...
try {
    ...
    adiwira_redirect_with_flash($returnTo, 'success', 'Artikel berhasil direstore.');

} catch (Throwable $e) {
    error_log('bin/article/restore.php error: ' . $e->getMessage());
    adiwira_redirect_with_flash($returnTo, 'error', 'Gagal restore artikel.');
}