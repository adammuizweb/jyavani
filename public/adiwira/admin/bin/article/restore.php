...
if ($role === 'author' && (int)$post['created_by'] !== $uid) {
  qz_redirect($back . '&err=' . urlencode('Role kamu tidak punya akses restore artikel ini.'));
}

$pdo->prepare("UPDATE posts SET is_deleted=0, deleted_at=NULL, updated_at=NOW() WHERE id=:id AND type='article' LIMIT 1")
    ->execute([':id' => $id]);

qz_redirect($back . '&msg=' . urlencode('Artikel berhasil direstore ✅'));
