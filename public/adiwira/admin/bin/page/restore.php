...
$defaultReturnTo = '/adiwira/index.php?page=admin/bin/page/index';
...
if (!$row) {
    adiwira_redirect_with_flash($returnTo, 'error', 'Page tidak ditemukan di trash.');
}

if ($role === 'author' && (int)($row['created_by'] ?? 0) !== $uid) {
    adiwira_redirect_with_flash($returnTo, 'error', 'Role kamu tidak punya akses restore page ini.');
}
...
try {
    ...
    adiwira_redirect_with_flash($returnTo, 'success', 'Page berhasil direstore.');

} catch (Throwable $e) {
    error_log('bin/page/restore.php error: ' . $e->getMessage());
    adiwira_redirect_with_flash($returnTo, 'error', 'Gagal restore page.');
}