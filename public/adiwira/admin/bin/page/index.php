<?php
// /adiwira/admin/bin/page/index.php
...
<section class="adam-card">
  ...
  <form method="get" style="margin-bottom:1rem;display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
    ...
    <a href="<?= htmlspecialchars($root . '/index.php?page=admin/bin/page/index', ENT_QUOTES, 'UTF-8') ?>" class="adam-link">Reset</a>
  ...
  <?php if ($canBulk): ?>
    <form method="post" action="<?= htmlspecialchars($root . '/admin/bin/page/bulk_action.php', ENT_QUOTES, 'UTF-8') ?>" onsubmit="return confirmBinBulk()">
      ...
      <?php else: ?>
        <?php foreach ($rows as $p): ?>
          <tr>
            ...
            <td>
              <div style="font-weight:600;"><?= htmlspecialchars($p['title'] ?? '-', ENT_QUOTES, 'UTF-8') ?></div>
            </td>
            ...
            <td><?= htmlspecialchars(!empty($p['deleted_at']) ? format_date_ddmmyyyy_time_bracket($p['deleted_at']) : '-', ENT_QUOTES, 'UTF-8') ?></td>

            <td><?= htmlspecialchars(!empty($p['created_at']) ? format_date_ddmmyyyy_time_bracket($p['created_at']) : '-', ENT_QUOTES, 'UTF-8') ?></td>
...
function openBinModal(type, btn) {
  ...
  if (type === 'restore') {
    document.getElementById('binTitle').innerText = 'Restore Page';
    document.getElementById('binText').innerText = `Restore page "${title}"?`;
    document.getElementById('binSubmit').innerText = 'Restore';
    document.getElementById('binSubmit').style.background = '#246';
    form.action = "<?= htmlspecialchars($root . '/admin/bin/page/restore.php', ENT_QUOTES, 'UTF-8') ?>";
  } else {
    document.getElementById('binTitle').innerText = 'Hapus Permanen';
    document.getElementById('binText').innerText = `Hapus permanen page "${title}"? Ini tidak bisa dibatalkan.`;
    document.getElementById('binSubmit').innerText = 'Hapus Permanen';
    document.getElementById('binSubmit').style.background = '#c33';
    form.action = "<?= htmlspecialchars($root . '/admin/bin/page/delete_permanent.php', ENT_QUOTES, 'UTF-8') ?>";
  }
...
function confirmBinBulk() {
  ...
  if (!any) { alert('Pilih minimal satu page.'); return false; }

  if (action === 'delete_permanent') {
    return confirm('Yakin ingin menghapus permanen semua page terpilih? Ini tidak bisa dibatalkan.');
  }
  return true;
}
</script>
