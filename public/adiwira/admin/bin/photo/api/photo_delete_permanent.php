<?php
// /adiwira/admin/bin/photo/api/photo_delete_permanent.php
...
// pastikan item ada di trash
$st0 = $pdo->prepare("SELECT id FROM posts WHERE id=:id AND type='photo' AND is_deleted=1 LIMIT 1");
$st0->execute([':id'=>$id]);
if (!$st0->fetchColumn()) json_out(['ok'=>false,'error'=>'Item tidak ditemukan di trash.'], 404);
...
try {
  ...
  $st = $pdo->prepare("DELETE FROM posts WHERE id=:id AND type='photo' AND is_deleted=1 LIMIT 1");
  $st->execute([':id'=>$id]);
  ...
  json_out(['ok'=>false,'error'=>'Gagal hapus permanen: '.$e->getMessage()], 500);
}
