...
$current_title = function_exists('settings_get') ? (settings_get($pdo,'site_title','Pre  Univ APU') ?? 'Pre  Univ APU') : 'Pre  Univ APU';
$current_host  = function_exists('settings_get') ? (settings_get($pdo,'site_host','pre-univapu.kmb.ac.id') ?? 'pre-univapu.kmb.ac.id') : 'pre-univapu.kmb.ac.id';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($site_host !== '' && !preg_match('/^[a-z0-9][a-z0-9\.\-]*[a-z0-9]$/i', $site_host)) {
        $errors[] = 'Format host tidak valid. Contoh: pre-univapu.kmb.ac.id';
    }
...
<section class="adam-card" style="max-width:820px;margin:18px auto;">
  ...
  <form method="post" novalidate>
    ...
    <div style="margin-top:14px;display:flex;gap:10px;align-items:center;">
      ...
      <a class="adam-cancle" href="/adiwira/index.php?page=admin/settings/index">Kembali</a>
    </div>
  </form>
</section>
