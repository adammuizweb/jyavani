...
<?php if (!defined('PCAT_CARDS_CSS')): define('PCAT_CARDS_CSS', true); ?>
<style id="pcat-cards-css">
  .pcat--cards{
    --pcat-gap: var(--space-4, 16px);
  ...
  .pcat--cards .pcat__track::-webkit-scrollbar{ height: 8px; }
  .pcat--cards .pcat__track::-webkit-scrollbar-thumb{
    background: var(--border, rgba(0,0,0,.18));
    border-radius: 999px;
  }
  .pcat--cards .pcat__track::-webkit-scrollbar-track{ background: transparent; }

  .pcat--cards .pcat__item{ flex:0 0 auto; scroll-snap-align:start; }

  .pcat--cards .pcat__card{
    position:relative;
    display:block;
    ...
    color: #fff;
    background: var(--pcat-card-bg, #0b1220);
    box-shadow: 0 12px 40px var(--shadow, rgba(0,0,0,.18));
    transform: translateZ(0);
    transition: box-shadow .25s ease;
  }

  .pcat--cards .pcat__card:hover{
    box-shadow: 0 18px 50px rgba(0,0,0,.28);
  }

  .pcat--cards .pcat__media{ position:absolute; inset:0; }
  .pcat--cards .pcat__img{
  ...
  .pcat--cards .pcat__img--placeholder{
    width:100%; height:100%;
    background:
  ...
  .pcat--cards .pcat__date{
    font-size: 12px;
    color: rgba(255,255,255,.78);
  }

  .pcat--cards .pcat__card:hover .pcat__img{ transform: scale(1.08); }
  .pcat--cards .pcat__card:focus-visible{
  ...
  @media (max-width: 900px){
    .pcat--cards{ --pcat-card-w:300px; --pcat-card-h:420px; }
  }
  @media (max-width: 520px){
    .pcat--cards{ --pcat-card-w:78vw; --pcat-card-h:420px; }
    .pcat--cards .pcat__track{ gap: 12px; }
  }
</style>
<?php endif; ?>
