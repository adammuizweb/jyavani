...
// Estimasi waktu baca
$wordCount = str_word_count(safe_strip_tags($post['content']));
...
<?= json_encode([
  ...
  'description' => mb_strimwidth(safe_strip_tags($post['content'] ?? ''), 0, 160, '...')
...
<?php elseif (!empty($post['display_image'])): ?>
  <figure class="adam-post-thumb">
    <img src="<?= htmlspecialchars($post['display_image'], ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($post['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
  </figure>
<?php endif; ?>
