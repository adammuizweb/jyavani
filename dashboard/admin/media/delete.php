...
try {
    if ($id <= 0) {
        asset_lifecycle_delete_temporary_public($pdo, 'media', $url, $uid, (string)($_POST['cleanup_token'] ?? ''));
        adiwira_json(['ok' => true, 'operation' => 'purged', 'url' => $url, 'physical_deleted' => true]);
    }

    $result = asset_lifecycle_trash($pdo, 'media', [$id], $uid);
    $action = null;
    try {
        $action = adiwira_asset_issue_trash_undo($pdo, 'media', $uid, $result['items']);
    } catch (Throwable $undoError) {
        error_log('[media/delete] trash committed but Undo grant failed: ' . $undoError->getMessage());
    }
    adiwira_json([
        'ok' => true,
        'operation' => 'trashed',
    ...
    error_log('[media/delete] ' . $e->getMessage());
    adiwira_json(['ok' => false, 'error' => __('Failed to move media to trash.')], 500);
}
