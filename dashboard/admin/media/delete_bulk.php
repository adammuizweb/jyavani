...
try {
    $result = asset_lifecycle_trash($pdo, 'media', (array)($_POST['ids'] ?? []), $uid);
    $action = null;
    try {
        $action = adiwira_asset_issue_trash_undo($pdo, 'media', $uid, $result['items']);
    } catch (Throwable $undoError) {
        error_log('[media/delete_bulk] trash committed but Undo grant failed: ' . $undoError->getMessage());
    }
    adiwira_json([
        'ok' => true,
        'operation' => 'trashed',
    ...
    error_log('[media/delete_bulk] ' . $e->getMessage());
