...
try {
    ...
    // cek schema agar aman di server yang belum sinkron
    $hasSortOrder = false;
    $hasPostMediaItems = false;
    ...
    try {
        $stCol = $pdo->query("
            ...
            LIMIT 1
        ");
        $hasSortOrder = (bool)$stCol->fetchColumn();
    } catch (Throwable $e) {
        $hasSortOrder = false;
    }
    ...
    try {
        $stTbl = $pdo->query("
            ...
            LIMIT 1
        ");
        $hasPostMediaItems = (bool)$stTbl->fetchColumn();
    } catch (Throwable $e) {
        $hasPostMediaItems = false;
    }

    $mediaCountSql = $hasPostMediaItems
        ? "(SELECT COUNT(*) FROM post_media_items x WHERE x.post_id = p.id)"
        : "0";

    $sortOrderSelect = $hasSortOrder
        ? "p.sort_order"
        : "NULL AS sort_order";

    $orderSql = $hasSortOrder
        ? "CASE WHEN p.sort_order IS NULL THEN 0 ELSE 1 END DESC, p.sort_order ASC, p.id DESC"
        : "p.id DESC";
    ...
    out_json([
        ...
        'schema' => [
            ...
            'has_post_media_items' => $hasPostMediaItems,
        ],
    ]);
