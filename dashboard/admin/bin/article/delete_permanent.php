...
if (!$post) {
    adiwira_redirect_with_flash($returnTo, 'error', 'Artikel tidak ditemukan di trash.');
}

if ($role === 'author' && (int)($post['created_by'] ?? 0) !== $uid) {
    adiwira_redirect_with_flash($returnTo, 'error', 'Role kamu tidak punya akses hapus permanen artikel ini.');
}
...
try {
    ...
    adiwira_redirect_with_flash($returnTo, 'success', 'Artikel berhasil dihapus permanen.');
    ...
    adiwira_redirect_with_flash($returnTo, 'error', 'Gagal hapus permanen artikel.');
}