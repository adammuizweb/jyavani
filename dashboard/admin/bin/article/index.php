  ...
  function getBulkSummary(){
    ...
    if (action === 'restore') {
      return {
        ok: true,
        variant: 'warning',
        title: '<?=__('Restore selected articles')?>',
        message: count + <?= json_encode(__(' article(s) will be restored from trash. Continue?')) ?>,
    ...
    if (action === 'delete_permanent') {
      return {
        ok: true,
        variant: 'danger',
        title: '<?=__('Delete permanently articles')?>',
        message: count + <?= json_encode(__(' article(s) will be permanently deleted. This action cannot be undone.')) ?>,
  ...
  document.querySelectorAll('.js-bin-article-restore').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      ask('warning', {
        ...
        message: '<?=__('Restore article')?> "' + title + '" <?=__('from trash?')?>',
        confirmText: '<?=__('Yes, restore')?>',
  ...
  document.querySelectorAll('.js-bin-article-delete-permanent').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      ask('danger', {
        ...
        message: '<?=__('Delete permanently article')?> "' + title + '"? <?=__('This action cannot be undone.')?>',
        confirmText: '<?=__('Yes, delete permanently')?>',
        cancelText: <?= json_encode(__('Cancel')) ?>
      }).then(function(ok){
        if (!ok) return;
