...
if (!function_exists('respond_article_bin_bulk')) {
    function respond_article_bin_bulk(bool $ok, string $message = '', int $httpCode = 200, array $extra = [], ?string $redirect = null): void {
        $redirect = $redirect ?: ADMIN_BASE_PATH . '/?page=admin/bin/article/index';
...
if (!adiwira_csrf_validate($token)) {
    respond_article_bin_bulk(false, __('Invalid CSRF token.'), 419, [], $returnTo);
}

$ids = $_POST['ids'] ?? [];
if (!is_array($ids) || empty($ids)) {
    respond_article_bin_bulk(false, __('No articles selected.'), 400, [], $returnTo);
}

$ids = array_values(array_filter(array_map('intval', $ids), fn($v) => $v > 0));
if (empty($ids)) {
    respond_article_bin_bulk(false, __('Invalid article ID.'), 400, [], $returnTo);
}

$action = (string)($_POST['action'] ?? '');
if ($action === '') {
    respond_article_bin_bulk(false, __('Unknown bulk action.'), 400, [], $returnTo);
}
...
try {
    ...
    if ($action === 'restore') {
        ...
        $pdo->commit();
        respond_article_bin_bulk(true, "Successfully restored  {$affected} artikel.", 200, ['count' => $affected], $returnTo);
    }
    ...
    if ($action === 'delete_permanent') {
        ...
        $pdo->commit();
        respond_article_bin_bulk(true, "Permanently deleted  {$affected} artikel.", 200, ['count' => $affected], $returnTo);
    }
