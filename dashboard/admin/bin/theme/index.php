...
<section class="adam-card">
  ...
  <form method="get" style="margin-bottom:1rem;display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
    ...
    <a href="<?= htmlspecialchars($base . '/?page=admin/bin/theme/index', ENT_QUOTES, 'UTF-8') ?>" class="adam-cancle">Reset</a>
          ...
          <?php else: ?>
            <?php foreach ($themes as $t): ?>
              ...
              <tr class="adam-row">
                ...
                <td><?= htmlspecialchars(!empty($t['deleted_at']) ? format_date_ddmmyyyy_time_bracket((string)$t['deleted_at']) : '-', ENT_QUOTES, 'UTF-8') ?></td>
                <td><?= htmlspecialchars(!empty($t['created_at']) ? format_date_ddmmyyyy_time_bracket((string)$t['created_at']) : '-', ENT_QUOTES, 'UTF-8') ?></td>
  ...
  <form id="bin-theme-restore-form" method="post" action="<?= htmlspecialchars($base . '/admin/bin/theme/restore.php', ENT_QUOTES, 'UTF-8') ?>" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" name="id" id="bin-theme-restore-id">
    <input type="hidden" name="return_to" id="bin-theme-restore-return-to" value="<?= htmlspecialchars($currentReturnTo, ENT_QUOTES, 'UTF-8') ?>">
  </form>

  <form id="bin-theme-delete-form" method="post" action="<?= htmlspecialchars($base . '/admin/bin/theme/delete_permanent.php', ENT_QUOTES, 'UTF-8') ?>" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" name="id" id="bin-theme-delete-id">
    <input type="hidden" name="return_to" id="bin-theme-delete-return-to" value="<?= htmlspecialchars($currentReturnTo, ENT_QUOTES, 'UTF-8') ?>">
  </form>
</section>
...
(function(){
  ...
  const restoreReturnTo = document.getElementById('bin-theme-restore-return-to');

  const deleteForm = document.getElementById('bin-theme-delete-form');
  const deleteId = document.getElementById('bin-theme-delete-id');
  const deleteReturnTo = document.getElementById('bin-theme-delete-return-to');
  ...
  function getBulkSummary(){
    ...
    if (action === 'restore') {
      return {
        ...
        message: 'Sebanyak ' + count + ' theme akan direstore dari trash. Lanjutkan?',
    ...
    if (action === 'delete_permanent') {
      return {
        ...
        message: 'Sebanyak ' + count + ' theme akan dihapus permanen. Aksi ini tidak bisa dibatalkan.',
