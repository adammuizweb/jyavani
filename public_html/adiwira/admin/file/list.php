...
.file-thumb { width:72px; height:48px; display:flex; align-items:center; justify-content:center; background:#f7f7f7; border-radius:6px; border:1px solid #eee; font-weight:700; color:#333; }
      ...
      <tbody>
        <?php foreach ($rows as $r): ?>
          <tr data-id="<?= (int)$r['id'] ?>">
            ...
            <td>
              <div class="file-thumb"><?= e(strtoupper($r['ext'] ?? pathinfo($r['filename'] ?? '', PATHINFO_EXTENSION) ?: '')) ?></div>
            </td>
            <td>
            ...
            <td>
              ...
              <?php if (!empty($r['media_type'])): ?>
                <div class="small">Type: <?= e($r['media_type']) ?></div>
              <?php endif; ?>
            ...
            <td>
              ...
              <a class="btn" href="<?= e($r['url']) ?>" target="_blank" rel="noopener">Download</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
...
(function(){
  ...
  // reload fragment with q and p
  async function reloadListFragment(q = '', p = 1) {
    try {
      const url = '/adiwira/admin/file/list.php' + (q ? ('?q=' + encodeURIComponent(q) + '&p=' + encodeURIComponent(p)) : ('?p=' + encodeURIComponent(p)));
      ...
      alert('Gagal memuat daftar file: ' + err.message);
    }
  }
  ...
  document.addEventListener('click', async function(ev){
    ...
    if (t.matches('#delete-bulk-btn')) {
      ...
      if (checked.length === 0) return alert('Pilih minimal satu file untuk dihapus.');
      ...
      try {
        ...
        if (j.ok) {
          ...
          document.dispatchEvent(new CustomEvent('file:deleted', { detail: { ids: checked, result: j } }));
          // remove rows immediately
          checked.forEach(id => {
            const tr = (container || document).querySelector('tr[data-id="'+id+'"]');
            if (tr) tr.remove();
          });
          // reload current page to refresh total/pagination
  ...
  if (!window.__fileListDelegationInstalled) {
    console.log('File list: pagination & delegation installed.');
