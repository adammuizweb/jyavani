<?php
// /adiwira/admin/bin/page/restore.php
...
$stmt = $pdo->prepare("SELECT id, created_by FROM posts WHERE id=:id AND type='page' AND is_deleted=1 LIMIT 1");
$stmt->execute([':id' => $id]);
$row = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$row) qz_redirect($back . '&err=' . urlencode('Page tidak ditemukan di trash.'));

if ($role === 'author' && (int)$row['created_by'] !== $uid) {
  qz_redirect($back . '&err=' . urlencode('Role kamu tidak punya akses restore page ini.'));
}

$pdo->prepare("UPDATE posts SET is_deleted=0, deleted_at=NULL, updated_at=NOW() WHERE id=:id AND type='page' LIMIT 1")
    ->execute([':id' => $id]);

qz_redirect($back . '&msg=' . urlencode('Page berhasil direstore ✅'));
