...
if ($role === 'author' && (int)$row['created_by'] !== $uid) {
  qz_redirect($back . '&err=' . urlencode('Role kamu tidak punya akses hapus permanen page ini.'));
}

try {
  $pdo->beginTransaction();

  // aman saja (kalau ada relasi nyangkut)
  $pdo->prepare("DELETE FROM post_categories WHERE post_id=:id")->execute([':id' => $id]);

  $pdo->prepare("DELETE FROM posts WHERE id=:id AND type='page' AND is_deleted=1 LIMIT 1")
      ->execute([':id' => $id]);

  $pdo->commit();
  qz_redirect($back . '&msg=' . urlencode('Page berhasil dihapus permanen 🧨'));
} catch (Throwable $e) {
