/* /static/assets/js/anime.js
   ...
   - triggers: load / scroll / manual
   - supports old attrs: data-delay, data-duration, data-ease, data-repeat, data-once
   - supports new attrs: data-anim-*, data-wave-*, data-ml-*
   - safe wave-span: will NOT destroy DOM unless target is text-only
   - auto refresh via MutationObserver
   ========================================================== */
...
(() => {
  ...
  function prepareWaveSpan(el){
    ...
    if (!ok) {
      log('wave-span target not text-only, skipped:', target);
      el.__wavePrepared = true;
      return;
    }
