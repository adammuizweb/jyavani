...
(() => {
  ...
  function prepareFragReveal(el){
    ...
    // hide lebih awal khusus trigger load (anti flash)
    if (trig === 'load') {
      el.classList.add('fx-frag-prepared');
      setTimeout(() => {
        if (!el.__fragBuilt && !el.__fragDone) el.classList.remove('fx-frag-prepared');
      }, 3000);
    }
    ...
    function build(){
      ...
      const finalize = () => {
        ...
        if (cleanup && el.__fragOverlay) {
          ...
          ov.addEventListener('transitionend', removeOverlay, { once: true });
